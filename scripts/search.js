let search_gifos_container = document.getElementsByClassName(
  "search-gifos-container"
)[0];
...
let search_without_result_container = document.getElementsByClassName(
  "search-without-result-container"
)[0];
let search_without_result_title = document.getElementsByClassName(
...
search_input[0].addEventListener("keyup", (evento) => {
  if (evento.keyCode == key_code_enter) {
    ...
    let url_autocomplete =
      "https://api.giphy.com/v1/gifs/search/tags?" + q + "?&" + api_key;
...
let searchGifo = () => {
  ...
  let q = "q=" + search_input[0].value;
  let url_search_gifos =
    "https://api.giphy.com/v1/gifs/search?" + q + "?&" + api_key;
    ...
    .then((response_search) => {
      for (let i = 0; i < response_search.data.length; i++) {
        let gifo = new New_gifo(
          response_search.data[i],
          "gifo-search-" + (i + 1),
          "fav-icon-gs-" + (i + 1),
          "fav-icon-act-gs-" + (i + 1),
          "dow-icon-gs-" + (i + 1),
          "full-screen-icon-gs-" + (i + 1),
          "trash-icon-gs-" + (i + 1)
        );
...
let add_gifo_cards_finded = () => {
  if (gifos_searched_array.length != 0) {
    ...
    if (num_gifos_results <= gifos_searched_array.length) {
      let lim_sup =
        num_gifos_results + 12 <= gifos_searched_array.length
          ? num_gifos_results + 12
          : num_gifos_results +
            (gifos_searched_array.length - num_gifos_results);
      for (let i = num_gifos_results; i < lim_sup; i++) {
        add_gifo_card(
          results_gifos_section,
          "gifo-searched",
          gifos_searched_array,
          i
        );
      }
