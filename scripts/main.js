...
let api_key = "api_key=w3JmbZQdD4ttrpnH2v9GaK8TDXmvDRvb";
...
let full_screen_icon_fav_act =
  document.getElementsByClassName("fs-fav-act-box");
let full_screen_icon_fav = document.getElementsByClassName(
  "icon-fav-full-screen"
);
let full_screen_icon_dow = document.getElementsByClassName(
  "icon-dow-full-screen"
);
...
let trending_gifos_section = document.getElementsByClassName(
  "trending-gifos-container"
)[0];
  ...
  fetch(url_trending)
    ...
    .then((gifo_response) => {
      for (let i = 0; i < num_gifos_slider; i++) {
        let gifo = new New_gifo(
          gifo_response.data[i],
          "gifo-trending-" + (i + 1),
          "fav-icon-gt-" + (i + 1),
          "fav-icon-act-gt-" + (i + 1),
          "dow-icon-gt-" + (i + 1),
          "full-screen-icon-gt-" + (i + 1),
          "trash-icon-gt-" + (i + 1)
        );
...
let add_gifo_card = (parent_node, specific_class, gifos_list, i) => {
  ...
  let gifo_img = document.querySelectorAll(
    "div.gifos-box." + specific_class + " >label> img.gifo-img"
  );
  gifo_img[i].src = gifos_list[i].url_gif.images.original.url;
  let title_gifo = document.querySelectorAll(
    "div.gifos-box." + specific_class + ">div.gifos-box-hover>p.title-gifos"
  );
  title_gifo[i].innerHTML = gifos_list[i].url_gif.title;
  let user_gifo = document.querySelectorAll(
    "div.gifos-box." + specific_class + ">div.gifos-box-hover>p.user-gifos"
  );
  user_gifo[i].innerHTML =
    gifos_list[i].url_gif.username == ""
      ? "GIFOS User"
      : gifos_list[i].url_gif.username;
  gifo_img[i].id = gifos_list[i].id_gif_img;
  let gifo_trash_icon = document.querySelectorAll(
    "div.gifos-box." +
      specific_class +
      "> div.gifos-box-hover> div.icons-gifos-box> div.delete-icon"
  );
  gifo_trash_icon[i].id = gifos_list[i].id_trash_icon;
  let gifo_fav_icon = document.querySelectorAll(
    "div.gifos-box." +
      specific_class +
      "> div.gifos-box-hover> div.icons-gifos-box> div.icon-fav"
  );
  gifo_fav_icon[i].id = gifos_list[i].id_fav_gif_icon;
  let gifo_fav_icon_act = document.querySelectorAll(
    "div.gifos-box." +
      specific_class +
      "> div.gifos-box-hover> div.icons-gifos-box> div.fav-icon-activate"
  );
  gifo_fav_icon_act[i].id = gifos_list[i].id_fav_gif_activate_icon;
  let gifo_dow_icon = document.querySelectorAll(
    "div.gifos-box." +
      specific_class +
      "> div.gifos-box-hover> div.icons-gifos-box> div.icon-download"
  );
  gifo_dow_icon[i].id = gifos_list[i].id_dow_gif_icon;
  gifo_fs_icon = document.querySelectorAll(
    "div.gifos-box." +
      specific_class +
      "> div.gifos-box-hover> div.icons-gifos-box> div.full-screen-icon"
  );
...
let gifoFullScreenDesktop = (gifo) => {
  if (gifo.id.slice(0, 19) == "full-screen-icon-gt") {
    let index = trending_gifos_array.findIndex(
      (x) => x.id_fs_gif_icon == gifo.id
    );
    gifo_full_screen(index, trending_gifos_array);
  } else if (gifo.id.slice(0, 19) == "full-screen-icon-gs") {
    let index = gifos_searched_array.findIndex(
      (x) => x.id_fs_gif_icon == gifo.id
    );
    gifo_full_screen(index, gifos_searched_array);
    ...
    let index = created_gifos_array.findIndex(
      (x) => x.id_fs_gif_icon == gifo.id
    );
    gifo_full_screen(index, created_gifos_array);
  }
...
let gifo_full_screen = (index, gifos_list) => {
  full_screen_gifo.setAttribute(
    "src",
    gifos_list[index].url_gif.images.original.url
  );
  full_screen_user.textContent =
    gifos_list[index].url_gif.username == ""
      ? "GIFOS User"
      : gifos_list[index].url_gif.username;
  ...
  let ind = favorite_array.findIndex(
    (x) => x.url_gif.id == gifos_list[index].url_gif.id
  );
  if (ind != -1) {
  ...
  let index_Trash = created_gifos_array.findIndex(
    (x) => x.url_gif.id == gifos_list[index].url_gif.id
  );
  if (index_Trash != -1) {
    full_screen_icon_fav[0].style.display = "none";
...
let addFavoriteGifo = (icon_fav) => {
  if (icon_fav.id.slice(0, 12) == "fav-icon-gt-") {
    let index = trending_gifos_array.findIndex(
      (x) => x.id_fav_gif_icon == icon_fav.id
    );
...
let fav_bttn_efect = (gifo_type, g_type, index) => {
  let btn_fav_normal = document.querySelectorAll(
    "div." +
      gifo_type +
      "> div.gifos-box-hover > div.icons-gifos-box > div#fav-icon-" +
      g_type +
      "-" +
      (index + 1)
  );
  btn_fav_normal[0].style.display = "none";
  let btn_fav_activate = document.querySelectorAll(
    "div." +
      gifo_type +
      "> div.gifos-box-hover > div.icons-gifos-box > div#fav-icon-act-" +
      g_type +
      "-" +
      (index + 1)
  );
  btn_fav_activate[0].style.display = "inline-block";
  let btn_fav_normal_fs = document.querySelectorAll(
    "div.full-screen-container > div.icon-fav-full-screen "
  );
  btn_fav_normal_fs[0].style.display = "none";
  let btn_fav_active_fs = document.querySelectorAll(
    "div.full-screen-container > div.fs-fav-act-box "
  );
...
let display_fav_act_icon = (gifo, i) => {
  ...
  if (index != -1) {
    ...
    let fav_icon_normal = document.querySelectorAll(
      "div#" + id_gifo_fav + (i + 1)
    );
    fav_icon_normal[0].style.display = "none";
    let id_gifo_fav_act = gifo.id_fav_gif_activate_icon.slice(0, 16);
    let fav_icon_act = document.querySelectorAll(
      "div#" + id_gifo_fav_act + (i + 1)
    );
...
let download_gifo = (gifo) => {
  if (gifo.id.slice(0, 12) == "dow-icon-gt-") {
    ...
    let id_download =
      created_gifos_array[created_gifos_array.length - 1].url_gif.id;
    let title_download = "My_Gifo";
    download_gifo_id(title_download, id_download);
  }
};
...
let see_more_button_myGifos = document.getElementById(
  "see-more-button-myGifos"
);
...
let add_my_gifo_cards = () => {
  if (created_gifos_array.length != 0) {
    ...
    if (created_gifos_array.length > 12) {
      ...
      let lim_sup =
        num_gifos_myGifos + 12 <= created_gifos_array.length
          ? num_gifos_myGifos + 12
          : num_gifos_myGifos +
            (created_gifos_array.length - num_gifos_myGifos);
      for (let i = num_gifos_myGifos; i < lim_sup; i++) {
        ...
        let fav_icon_mygifos = document.getElementById(
          "fav-icon-gc-" + (i + 1)
        );
        fav_icon_mygifos.style.display = "none";
        let trash_icon_mygifos = document.getElementById(
          "trash-icon-gc-" + (i + 1)
        );
