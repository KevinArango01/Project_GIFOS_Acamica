...
let create_gifos_section = document.getElementsByClassName(
...
let cam_screen_container = document.getElementsByClassName(
...
let search_gifo_byId = (gif_id) => {
  let url_search_gifos_by_ID =
    "https://api.giphy.com/v1/gifs/" + gif_id + "?&" + api_key;
  fetch(url_search_gifos_by_ID)
    ...
    .then((data_new_gifo) => {
      ...
      let gifo = new New_gifo(
        ...
        "gifo-created-" + (long + 1),
        "fav-icon-gc-" + (long + 1),
        "fav-icon-act-gc-" + (long + 1),
        "dow-icon-gc-" + (long + 1),
        "full-screen-icon-gc-" + (long + 1),
        "trash-icon-gc-" + (long + 1)
      );
