let favorite_gifos_section = document.getElementsByClassName(
  "favorite-gifos-section"
)[0];
let favorite_gifos_container = document.getElementById(
  "favorite-gifos-container"
);
let without_favorite_cotainer = document.getElementsByClassName(
  "without-favorites-container"
)[0];
let gifo_box_favorite = document.getElementsByClassName("gifo-favorite");
let see_more_button_favorite = document.getElementById(
  "see-more-button-favorite"
);
...
let add_fav_gifo = (url_gifo_fav) => {
  ...
  let gifo = new New_gifo(
    url_gifo_fav,
    "gifo-favorite-" + (long + 1),
    "fav-icon-gf-" + (long + 1),
    "fav-icon-act-gf-" + (long + 1),
    "dow-icon-gf-" + (long + 1),
    "full-screen-icon-gf-" + (long + 1),
    "trash-icon-gf-" + (long + 1)
  );
...
let add_fav_gifo_cards = () => {
  if (favorite_array.length != 0) {
    ...
    if (favorite_array.length > 12) {
      ...
      let lim_sup =
        num_gifos_favorites + 12 <= favorite_array.length
